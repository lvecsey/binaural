...
int main(int argc, char *argv[]) {
  ...
  for (sampleno = 0; sampleno < num_samples; sampleno++) {
    ...
    v = 0.5 + (0.5 * sin(2.0 * M_PI * freq * sampleno / (num_samples - 1)));
    
    radians = (2.0 * M_PI * sampleno * stereo_freqs[0] * duration) / (num_samples - 1);
    preres[0] = sin(radians);

    radians = (2.0 * M_PI * sampleno * stereo_freqs[1] * duration) / (num_samples - 1);
    preres[1] = sin(radians);

    samples[sampleno].channels[LEFT] = vol * ((v * preres[0]) + (1.0 - v) * preres[1]);

    samples[sampleno].channels[RIGHT] = vol * (((1.0 - v) * preres[0]) + v * preres[1]);
